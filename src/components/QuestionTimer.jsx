...
const QuestionTimer = ({ onTimeUp, setTimePerQuestion, isAnswered, resetTimer }) => {
  const [seconds, setSeconds] = useState(59); // 10 seconds for each question
  ...
  useEffect(() => {
    setSeconds(59); // Reset the timer to 10 seconds
  }, [resetTimer]); // `resetTimer` is a prop that changes when a new question is loaded

  return <div className="text-lg">Осталось времени: {seconds}s</div>;
};
